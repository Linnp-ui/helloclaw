...
class EnhancedSimpleAgent(SimpleAgent):
    ...
    def _build_messages(
        ...
        for msg in self._history:
            ...
            # 包含 metadata（包含 tool_calls 或 tool_call_id）
            if msg.metadata:
    ...
    async def arun_stream_with_tools(
        ...
        try:
            ...
            if not self._supports_streaming_tools:
                ...
                yield StreamEvent.create(
                    StreamEventType.AGENT_FINISH, self.name, result=response
                )
                return
            ...
            print(
                f"🔧 已启用工具调用，可用工具: {list(self.tool_registry._tools.keys())}"
            )
            ...
            while current_iteration < self.max_tool_iterations:
                ...
                try:
                    ...
                    print(f"\n❌ {error_msg}")
                    yield StreamEvent.create(
                        StreamEventType.ERROR, self.name, error=error_msg
                    )
                    break
                ...
                # 发送步骤完成事件
                yield StreamEvent.create(
                    StreamEventType.STEP_FINISH, self.name, step=current_iteration
                )
            ...
            if current_iteration >= self.max_tool_iterations and not final_response:
                ...
                try:
                    ...
                    result = self.llm.get_last_stream_tool_result()
                    final_response = (
                        result.content if result else "抱歉，我无法回答这个问题。"
                    )
            ...
            if tool_call_records:
                ...
                for i, tc in enumerate(tool_call_records):
                    self.add_message(
                        Message(
                            tc["result"], "tool", metadata={"tool_call_id": f"call_{i}"}
                        )
                    )
