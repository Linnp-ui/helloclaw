...
class ExecuteCommandTool(Tool):
    ...
    def _validate_command(self, command: str) -> tuple[bool, str]:
        ...
        if base_cmd not in self.allowed_commands:
            return (
                False,
                f"命令 '{base_cmd}' 不在白名单中。允许的命令: {', '.join(self.allowed_commands[:10])}...",
            )
    ...
    def _execute_command(
        ...
        if not is_safe:
            return ToolResponse.error(
                code="COMMAND_BLOCKED", message=f"命令被拦截: {reason}"
            )
        ...
        if workdir:
            ...
            if not is_valid:
                return ToolResponse.error(
                    code="DIRECTORY_NOT_ALLOWED", message=f"工作目录无效: {reason}"
                )
        ...
        try:
            ...
            return ToolResponse.error(
                code="TIMEOUT", message=f"命令执行超时（{timeout or self.timeout}秒）"
            )
        except Exception as e:
            return ToolResponse.error(
                code="EXECUTION_ERROR", message=f"命令执行失败: {str(e)}"
            )
    ...
    def _list_dangerous_patterns(self) -> str:
        """列出所有会被拦截的危险命令模式"""
        return "危险命令模式:\n" + "\n".join(
            f"- {pattern}" for pattern in self.dangerous_patterns
        )
